...
async function callGemini(base64Array) {
  ...
  const prompt = `這是日本收據照片${multi ? `（共 ${base64Array.length} 張，為同一張收據的不同部分，請合併所有照片的資訊一起辨識）` : ''}。

═══ STEP 1：讀取基本資訊 ═══
- storeName：收據最上方的店名（繁體中文翻譯，括號保留日文原文）
- date：收據上的日期，格式 YYYY-MM-DD；無法辨識填 ""
- total：收據最下方的【最終實付合計金額】（お支払い合計、合計、請求合計等）

═══ STEP 2：讀取商品明細 ═══
每一行商品：
- name：繁體中文翻譯（括號保留日文原文），絕對不能把「×2」「2個」等數量寫進名稱裡
- quantity：購買數量（整數，預設 1）；收據上的「2点」「×2」「2個」都要填入 quantity，不能放在 name
- price：【單價】= 該商品的單一售價（正整數）
  ⚠️ 收據若顯示「商品名 2個 ¥366」→ quantity=2, price=183（366÷2）
  ⚠️ 收據若顯示「商品名 ¥366 ×2」→ quantity=2, price=183（×2 代表數量，不是 name 的一部分）
  ⚠️ 收據若顯示「商品名 2点 ×¥5,580 = ¥11,160」→ quantity=2, price=5580（¥5,580 是單價，¥11,160 是行小計，price 填單價）
  ⚠️ App 計算：price × quantity = 行小計，絕對不能用行小計當 price
  ❌ 禁止：name="合力他命EX×2", quantity=1, price=5580 → 這樣只算 ¥5,580，漏掉一個

═══ STEP 3：判斷稅金方式（最關鍵）═══

先計算：商品小計 = Σ(price × quantity)
再扣除折扣：含折扣小計 = 商品小計 - Σ(discounts)

【判斷 A】若 含折扣小計 ≈ total（差距在 ±2 以內）
  → 内税（税込）：稅已含在商品價格中
  → tax8=0, tax10=0
  → 收據底部的「内消費税」「うち消費税」「（内訳）消費税8%/10%」只是說明含在其中的金額，絕對不要填入 tax8/tax10
  → 適用：便利商店、超市、藥妝店、居酒屋、多數餐廳、景點門票

【判斷 B】若 含折扣小計 + 消費税記載金額 ≈ total
  → 外税（税抜き）：稅是額外加上的
  → tax8 = 收據上明示的 8% 消費税額（整數）
  → tax10 = 收據上明示的 10% 消費税額（整數）
  → 適用：部分百貨公司、outlet 品牌店、部分高級餐廳
