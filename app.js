...
async function callGemini(base64Array) {
  ...
  const prompt = `這是日本收據照片${multi ? `（共 ${base64Array.length} 張，為同一張收據的不同部分，請合併所有照片的資訊一起辨識）` : ''}。請嚴格遵守以下規則後，只輸出一個 JSON。
...
━━━ STEP 1：基本資訊 ━━━
storeName：收據最上方的店名（繁體中文，括號保留日文）
date：YYYY-MM-DD；無法辨識填 ""
total：收據最下方的【最終實付合計】（お支払い合計、合計、ご請求額）
  ❌ 禁止用「小計」「商品計」「税抜合計」當 total，那些是中間計算值

━━━ STEP 2：商品明細（最重要）━━━
收據上的【每一行商品 = JSON 的獨立一筆】，絕對不能合併兩行為一筆。

◆ name：繁體中文翻譯（括號保留日文），禁止把數量「×2」「2個」等寫入 name
◆ quantity：收據上的購買數量（整數，預設 1）
◆ price：【單價】= 該商品單一個的售價（正整數）

【數量格式對照表 — 必須熟記】
  「商品名 2個 ¥730」         → quantity=2, price=365（730÷2）
  「商品名 ×2 ¥730」          → quantity=2, price=365（730÷2）
  「商品名 2点 @¥5,580 ¥11,160」→ quantity=2, price=5580（@ 後是單價，最後是行小計）
  「@¥5,580 ×2 = ¥11,160」   → quantity=2, price=5580
  「商品名 ¥5,580 (2)」       → quantity=2, price=5580
  ❌ 禁止：quantity=1, price=11160（把行小計當 price）
  ❌ 禁止：name="合力他命×2", quantity=1（把數量混進 name）

━━━ STEP 3：折扣明細 ━━━
◆ amount：折扣金額（【正整數】，不含負號；收據寫 -¥336 → amount=336）
◆ 只列「實際扣款」的折扣：クーポン割引、ポイント利用、割引、値引き
  ❌ 不列「集點/累積」：ポイント付与、ポイント積立（這是下次才能用的點數，不算這次折扣）
◆ 折扣 amount 對應的是【行小計】，不是單價
  → アリナミン 2個×¥5,580=¥11,160，クーポン3%=¥336（= ¥11,160×3%，不是 ¥5,580×3%）
◆ itemName 規則：
  → 折扣行緊接在某商品下方 → itemName = 上方商品的完整 name（一字不差複製）
  → 整張收據的折扣（出現在所有商品之後）→ itemName = ""

━━━ STEP 4：稅金判斷 ━━━
先算：A = Σ(price × quantity) - Σ(discounts.amount)

【A ≈ total（±5以內）】→ 内税，tax8=0, tax10=0, taxFree=0
  → 收據底部「内消費税 ¥XX」「(内訳)消費税」只是參考，絕不填入 tax8/tax10

【A + 消費税額 ≈ total】→ 外税，tax8/tax10 填收據上明示的金額

【taxFree 判斷（免税購物）】
  → 若 A ≈ total：taxFree=0（品項已是免税後售價，免税額只是參考）
  → 若 A - 免税額 ≈ total：taxFree=免税額
  ❌ 禁止：看到「(内消費税等) ¥XX」就設 taxFree=XX

━━━ STEP 5：驗算 ━━━
確認：Σ(price×quantity) - Σ(discounts.amount) + tax8 + tax10 + serviceCharge - taxFree ≈ total（±2）
若差距超過 ±2：
  → 優先懷疑某品項的 price 填了行小計而非單價（最常見）
  → 或 quantity 填錯（例如 2個卻填 1）
  → 或 discount.amount 填了負數

多張照片提醒：同一品項若在多張照片都出現，只計算一次，不重複列入 items。
